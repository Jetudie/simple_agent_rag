...
1. **Model API Update (Ollama)**: Update defaults throughout the codebase (`main.py`, `react.py`, `manager.py`) to point to `ollama/gemma4:e4b` utilizing LiteLLM.
...
> 1. **Model name verification**: You requested `gemma4:e4b`. Assuming Ollama is installed and running, LiteLLM accepts `ollama/gemma4:e4b`. Are you certain about the model tag? (Often Ollama tags are like `qwen2.5:3b`, etc., I will use exactly what you requested but please ensure the model is pulled locally).
...
### LLM Defaults Update
#### [MODIFY] `main.py`
Change model initializations to `ollama/gemma4:e4b` and remove explicit GEMINI_API warnings.
